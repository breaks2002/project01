  ...
  const indicatorCategories = useMemo(() => {
    ...
    data.forEach(d => {
      ...
      if (!categories.has(indicatorName)) {
        // 根据指标类型分配颜色（支持中文关键词）
        const name = d.nodeName;
        let color = '#6b7280'; // 默认灰色
        if (name.includes('净利润') || name.includes('利润') || name.includes('收益')) color = '#1e40af';
        else if (name.includes('收入') || name.includes('销售') || name.includes('流量')) color = '#16a34a';
        else if (name.includes('成本') || name.includes('损耗') || name.includes('损失')) color = '#dc2626';
        else if (name.includes('毛利') || name.includes('贡献')) color = '#9333ea';
        else if (name.includes('费用') || name.includes('税金')) color = '#ea580c';
        else if (name.includes('转化') || name.includes('率')) color = '#0891b2';
        else if (name.includes('数') || name.includes('量')) color = '#f59e0b';
    ...
    <div className="px-4 py-2 border-t bg-gray-50 flex items-center gap-6 flex-wrap">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          {/* 初始版本：只显示一个（初始数据与方案无关） */}
          <div className="flex items-center gap-1">
            <div className="w-3 h-3 rounded-full" style={{
              backgroundColor: '#6b7280',
              opacity: 0.4
            }} />
            <span className="text-xs text-gray-500">初始</span>
          </div>
          {/* 各方案当前版本 */}
