  ...
  const configMap = {
    ...
    increase: {
      ...
      valueTransform: (value, unitType) => {
        ...
        // 绝对额增量：提升50万 = 在当前值基础上 +50万
        return { delta: value, deltaType: 'absolute', constraintType: 'delta_absolute' };
      }
    },
    decrease: {
      patterns: [
  ...
  for (const [patternName, patternConfig] of Object.entries(patterns)) {
    ...
    allNodes.forEach(node => {
      ...
      allNames.forEach(factorName => {
        ...
        // 逐个 pattern 匹配，每个 pattern 单独判断是否含 {unit}
        patternConfig.patterns.forEach(patternTemplate => {
          const thisPatternHasUnit = patternTemplate.includes('{unit}');

          if (!thisPatternHasUnit) {
            // 无 {unit} 的 pattern，直接匹配（裸数字）
            ...
            if (match && match[1]) {
              ...
              // 无单位 pattern：默认为 yuan（原始数字）
              const unitTypeResolved = 'yuan';

              console.log(`[规则引擎] 匹配成功（无单位）！因子：${factorName}, 模式：${patternName}, 值：${value}, 单位：${unitTypeResolved}`);

              const constraint = patternConfig.valueTransform(value, unitTypeResolved);

              // 去重检查
              const constraintKey = `${node.id}|${patternConfig.type}|${value}|nounit`;
...
export const calculateTargetFromConstraint = (currentValue, constraint) => {
  ...
  switch (unitType) {
    ...
    case 'hundred_million':
      // 绝对额：根据 action 判断是目标值还是增量
      const units = loadUnits();
      const multiplier = units[unitType]?.multiplier || 1;
      if (action === 'increase') {
        // 增加：在当前值基础上加
        targetValue = currentValue + value * multiplier;
      } else if (action === 'decrease') {
        // 减少：在当前值基础上减
        targetValue = currentValue - value * multiplier;
      } else {
        // 其他（如 control_limit）：直接用作目标值
