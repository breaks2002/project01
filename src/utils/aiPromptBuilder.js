...
const formatConstraintDescription = (c) => {
  ...
  switch (constraintType) {
    ...
    case 'delta_absolute':
      // 绝对额增量：在当前值基础上加减
      const absDelta = constraint.delta;
      const absDir = absDelta > 0 ? '增加' : '减少';
      return `在当前值基础上${absDir}${Math.abs(absDelta)}${c.unit || '万'}`;
...
/**
 * 计算季节性模式
 * @param {Object} timeData - 月度数据（支持新格式 202601-AC 和旧格式 1月实际）
...
const calculateSeasonality = (timeData) => {
  ...
  Object.entries(timeData).forEach(([key, value]) => {
    ...
    // 新格式: 202601-AC, 2026WK01-FC 等
    const newMatch = key.match(/^(\d{4,8})-(?:AC|FC|BU)/);
    if (newMatch) {
      const period = newMatch[1];
      if (/^\d{6}$/.test(period)) {
        monthIndex = parseInt(period.slice(4, 6)) - 1; // 202601 → 0
      } else if (/^\d{8}$/.test(period)) {
        monthIndex = parseInt(period.slice(4, 6)) - 1; // 20260115 → 0
      }
    }

    // 旧格式兼容: 1月实际, 2月预测
    if (monthIndex < 0) {
      const oldMatch = key.match(/(\d{1,2})月/);
      if (oldMatch) {
        monthIndex = parseInt(oldMatch[1]) - 1;
      }
    }
    ...
    if (monthIndex >= 0 && monthIndex < 12) {
      ...
      if (!isNaN(numVal)) monthlyData[monthIndex].push(numVal);
    }
  });
...
const calculateInitialValue = (node) => {
  // 新架构优先：summary.actualPlusForecast
  if (node.summary?.actualPlusForecast !== undefined) {
    return node.summary.actualPlusForecast;
  }
...
const buildMonthlySummary = (timeData) => {
  ...
  // 区分 AC 和 FC 数据（兼容新旧格式）
  const allKeys = Object.keys(timeData);
  const isNewFormat = allKeys.some(k => /^\d+-(?:AC|FC|BU)/.test(k));

  let actualKeys, forecastKeys;
  if (isNewFormat) {
    // 新格式: 202601-AC, 202601-FC
    actualKeys = allKeys.filter(k => k.endsWith('-AC'));
    forecastKeys = allKeys.filter(k => k.endsWith('-FC'));
  } else {
    // 旧格式: 1月实际, 1月预测
    actualKeys = allKeys.filter(k => k.includes('实际'));
    forecastKeys = allKeys.filter(k => k.includes('预测'));
  }

  // 过滤掉累计值 key
  const nonCumulativeKeys = allKeys.filter(k => !k.includes('cumulative'));
  const values = nonCumulativeKeys.map(v => parseFloat(timeData[v])).filter(v => !isNaN(v));
  ...
  return {
    ...
    cv: avg !== 0 ? Math.round((stdDev / avg) * 100 * 100) / 100 : 0,
...
/**
 * 计算敏感性分析（增强版）
 * 通过解析公式链计算每个驱动因子对目标指标的实际影响系数
 * @param {Object} nodes - 所有节点
 * @param {string} targetNodeId - 目标节点ID
 * @returns {Array} 敏感性分析结果（包含逆推参考值）
 */
const calculateSensitivity = (nodes, targetNodeId) => {
  ...
  console.log('[敏感性分析] 目标节点:', targetNode.name, '值:', targetValue);
  console.log('[敏感性分析] 驱动因子数:', drivers.length);

  // 构建公式链敏感性（从目标追溯到驱动因子）
  const sensitivityMap = {};

  // 递归计算敏感性系数
  const calculateSensitivityRecursive = (nodeId, accumulatedCoefficient = 1, depth = 0) => {
    ...
    if (node.type === 'driver') {
      // 驱动因子：记录敏感性系数
      const driverValue = node.value ?? node.baseline ?? 1;
      if (driverValue === 0) return;

      // 1%变化导致的目标变化 = accumulatedCoefficient * driverValue * 0.01
      const onePercentImpact = accumulatedCoefficient * driverValue * 0.01;
      const relativeImpact = targetValue !== 0 ? onePercentImpact / Math.abs(targetValue) : 0;

      if (!sensitivityMap[nodeId]) {
        sensitivityMap[nodeId] = {
          nodeId: nodeId,
          nodeName: node.name,
          currentValue: Math.round(driverValue * 100) / 100,
          coefficient: accumulatedCoefficient,
          onePercentImpact: Math.round(onePercentImpact * 100) / 100,
          relativeImpact: Math.round(relativeImpact * 10000) / 100, // 百分比
          formulaChain: [],
          depth
        };
      } else {
        // 多条路径到达同一驱动因子，累加系数
        sensitivityMap[nodeId].coefficient += accumulatedCoefficient;
        sensitivityMap[nodeId].onePercentImpact += onePercentImpact;
        sensitivityMap[nodeId].relativeImpact += relativeImpact;
      }

      console.log(`[敏感性分析] 驱动因子 ${node.name}: 系数=${accumulatedCoefficient}, 1%影响=${onePercentImpact.toFixed(2)}, 相对影响=${relativeImpact.toFixed(4)}%`);
      return;
    }
    ...
    const formula = node.formula;
    console.log(`[敏感性分析] 计算指标 ${node.name}: 公式=${formula}, 累积系数=${accumulatedCoefficient}`);

    // 解析公式中的依赖和系数
    // 支持格式：A * k, A / k, A + B, A - B, A - B - C 等

    // 匹配乘法公式：Y = X * k 或 Y = k * X
    const multiplyMatch = formula.match(/^([a-zA-Z_\u4e00-\u9fa5][a-zA-Z0-9_\u4e00-\u9fa5]*)\s*\*\s*([0-9.]+)$/);
    const reverseMultiplyMatch = formula.match(/^([0-9.]+)\s*\*\s*([a-zA-Z_\u4e00-\u9fa5][a-zA-Z0-9_\u4e00-\u9fa5]*)$/);

    if (multiplyMatch) {
      // Y = X * k → X变化1单位导致Y变化k单位，所以敏感性系数 = accumulatedCoefficient * k
      const [, depId, kStr] = multiplyMatch;
      const k = parseFloat(kStr);
      calculateSensitivityRecursive(depId, accumulatedCoefficient * k, depth + 1);
      return;
    }

    if (reverseMultiplyMatch) {
      // Y = k * X → 同上
      const [, kStr, depId] = reverseMultiplyMatch;
      const k = parseFloat(kStr);
      calculateSensitivityRecursive(depId, accumulatedCoefficient * k, depth + 1);
      return;
    }
    ...
    // 匹配除法公式：Y = X / k
    const divideMatch = formula.match(/^([a-zA-Z_\u4e00-\u9fa5][a-zA-Z0-9_\u4e00-\u9fa5]*)\s*\/\s*([0-9.]+)$/);
    if (divideMatch) {
      // Y = X / k → X变化1单位导致Y变化1/k单位
      const [, depId, kStr] = divideMatch;
      const k = parseFloat(kStr);
      calculateSensitivityRecursive(depId, accumulatedCoefficient / k, depth + 1);
      return;
    }

    // 匹配加法公式：Y = A + B + C 或 Y = A + B
    const addMatch = formula.match(/^([a-zA-Z_\u4e00-\u9fa5][a-zA-Z0-9_\u4e00-\u9fa5]*)\s*\+\s*([a-zA-Z_\u4e00-\u9fa5][a-zA-Z0-9_\u4e00-\u9fa5]*)\s*(\+\s*[a-zA-Z_\u4e00-\u9fa5][a-zA-Z0-9_\u4e00-\u9fa5]*)*$/);
    if (addMatch) {
      // 加法：每个依赖的系数都是1（不考虑其他因子变化）
      const deps = formula.match(/[a-zA-Z_\u4e00-\u9fa5][a-zA-Z0-9_\u4e00-\u9fa5]*/g) || [];
      deps.forEach(depId => {
        calculateSensitivityRecursive(depId, accumulatedCoefficient, depth + 1);
      });
      return;
    }

    // 匹配减法公式：Y = A - B - C 或 Y = A - B
    const subtractMatch = formula.match(/^([a-zA-Z_\u4e00-\u9fa5][a-zA-Z0-9_\u4e00-\u9fa5]*)\s*-\s*([a-zA-Z_\u4e00-\u9fa5][a-zA-Z0-9_\u4e00-\u9fa5]*)\s*(-\s*[a-zA-Z_\u4e00-\u9fa5][a-zA-Z0-9_\u4e00-\u9fa5]*)*$/);
    if (subtractMatch) {
      // 减法：第一个因子系数为正，后面为负
      const parts = formula.split('-').map(p => p.trim());
      parts.forEach((part, i) => {
        const depId = part.match(/[a-zA-Z_\u4e00-\u9fa5][a-zA-Z0-9_\u4e00-\u9fa5]*/)?.[0];
        if (depId) {
          const sign = i === 0 ? 1 : -1;
          calculateSensitivityRecursive(depId, accumulatedCoefficient * sign, depth + 1);
        }
      });
      return;
    }

    // 复杂公式：尝试提取所有依赖，给每个依赖相同系数（简化处理）
    const allDeps = formula.match(/[a-zA-Z_\u4e00-\u9fa5][a-zA-Z0-9_\u4e00-\u9fa5]*/g) || [];
    allDeps.forEach(depId => {
      if (nodes[depId]) {
        calculateSensitivityRecursive(depId, accumulatedCoefficient * 0.5, depth + 1); // 默认系数0.5
      }
    });
  };

  // 从目标节点开始递归计算
  calculateSensitivityRecursive(targetNodeId);

  // 转换为数组并计算逆推参考值
  const sensitivityResults = Object.values(sensitivityMap).map(item => {
    // 计算逆推参考值：如果目标需要增加 delta，该驱动因子需要增加多少？
    // 对于 Y = X * k 类公式：如果 Y 增加 delta，则 X 需增加 delta/k
    const inverseReference = item.coefficient !== 0 ? `增加 ${item.coefficient.toFixed(4)} 倍的调整量` : '';

    return {
      ...item,
      impact: item.relativeImpact,
      elasticity: item.coefficient,
      priority: Math.abs(item.relativeImpact) > 0.1 ? 'high' : Math.abs(item.relativeImpact) > 0.01 ? 'medium' : 'low',
      inverseReference
    };
  }).sort((a, b) => Math.abs(b.relativeImpact) - Math.abs(a.relativeImpact));

  console.log('[敏感性分析] 结果数:', sensitivityResults.length);
  ...
  "adjustments": [
    {
      ...
      "monthlyStrategy": "分期分配策略名称",
      "monthlyFactors": [1.0, 1.1, ...], // 分配系数数组，数量与预测期间数一致（不一定是12个）
      "confidence": 0.85
    }
  ],
  "expectedImpact": {
...
敏感性分析（含逆推参考值）：
${JSON.stringify(sensitivity.slice(0, 5).map(s => ({
  ...s,
  // 添加逆推提示
  inverseHint: s.inverseReference || `变化1%导致目标变化${s.relativeImpact?.toFixed(2) || '未知'}%`
})), null, 2)}
...
${sensitivity.slice(0, 5).map(s => {
  if (s.coefficient && s.coefficient !== 0) {
    // 简单公式可以直接给出逆推公式
    const absCoeff = Math.abs(s.coefficient);
    if (absCoeff === 0.75) {
      return `- 若${s.nodeName}需增加delta，则驱动因子需增加 delta / 0.75 = delta × 1.33`;
    } else if (absCoeff > 0 && absCoeff < 1) {
      return `- 若${s.nodeName}需增加delta，则驱动因子需增加 delta / ${absCoeff.toFixed(2)} = delta × ${(1/absCoeff).toFixed(2)}`;
    } else if (absCoeff >= 1) {
      return `- 若${s.nodeName}需增加delta，则驱动因子需增加 delta / ${absCoeff.toFixed(2)}（系数较大，影响直接）`;
    }
  }
  return `- ${s.nodeName}: 影响${s.relativeImpact?.toFixed(2) || '未知'}%`;
}).join('\n')}
...
- [ ] monthlyFactors是数字数组（数量与预测期间数一致）？
