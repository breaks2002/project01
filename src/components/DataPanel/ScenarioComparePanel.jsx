...
const ScenarioComparePanel = ({ scenarios, onClose, isMinimized, onToggleMinimize, onBringToFront, canExport, isTrial, getDisableReason }) => {
  ...
  const getNodeAggregatedData = (node) => {
    ...
    if (!isDriver) {
      // 计算指标：直接使用 node.value（架构适配器算好的全周期值）
      // 不再依赖 aggregateTimeData，因为 aggregateTimeData 只计算了预测期
      actualPlusForecastTotal = node.value ?? 0;
    } else if (hasMonthlyFunction) {
      // 驱动因子但包含 MONTHLY 函数的特殊情况
      actualPlusForecastTotal = node.value ?? 0;
    }
  ...
  // 格式化期间键显示，去掉 -FC/-BU/-AC 后缀和 _cumulative 后缀
  const formatPeriodKey = (key) => {
    return key.replace(/-(FC|BU|AC)$/, '').replace(/_cumulative$/, '');
  };

  // 从 timeData 中根据纯周期键查找对应的值（优先找 -FC 键，其次找任意匹配的键）
  const findTimeDataValue = (timeData, pureKey) => {
    if (!timeData || !pureKey) return null;

    // 1. 优先找纯周期键（计算指标的结果）
    if (timeData[pureKey] !== undefined && timeData[pureKey] !== null) {
      return timeData[pureKey];
    }

    // 2. 找纯周期键 + -FC 后缀（预测期）
    const fcKey = pureKey + '-FC';
    if (timeData[fcKey] !== undefined && timeData[fcKey] !== null) {
      return timeData[fcKey];
    }

    // 3. 找任意匹配的键（排除 _cumulative）
    const nonCumKey = Object.keys(timeData).find(k =>
      k.replace(/_cumulative$/, '').replace(/-(FC|BU|AC)$/, '') === pureKey &&
      !k.endsWith('_cumulative')
    );
    if (nonCumKey) {
      return timeData[nonCumKey];
    }

    // 4. 最后找带 _cumulative 的键
    const cumKey = Object.keys(timeData).find(k =>
      k.replace(/_cumulative$/, '') === pureKey || k === pureKey
    );
    if (cumKey) {
      return timeData[cumKey];
    }

    return null;
  };

  // 收集预测期间（只显示预测期，因为实际数一样）
  const forecastMonthKeys = useMemo(() => {
      ...
      Object.values(scenario.nodes || {}).forEach(node => {
        if (node.timeData) {
          Object.keys(node.timeData).forEach(key => {
            // 新数据格式：2026WK01-FC，旧数据格式：9 月预测
            // 只收集预测期键，提取纯周期键（先去掉 _cumulative 后缀，再去掉 -FC/-AC/-BU 后缀）
            if (key.includes("-FC") || key.includes("预测")) {
              const pureKey = key.replace(/_cumulative$/, '').replace(/-(FC|BU|AC)$/, '');
              monthSet.add(pureKey);
            }
          });
        }
      });
    ...
    return Array.from(monthSet).sort((a, b) => {
      ...
      const getSortKey = (key) => {
        // 月度：202601
        const mMonth = key.match(/^(\d{4})(\d{2})$/);
        if (mMonth) return `${mMonth[1]}${mMonth[2]}`;
        // 日度：20260101
        const mDay = key.match(/^(\d{4})(\d{2})(\d{2})$/);
        if (mDay) return `${mDay[1]}${mDay[2]}${mDay[3]}`;
        // 周度：2026WK01
        const mWeek = key.match(/^(\d{4})WK(\d{2})$/);
        if (mWeek) return `${mWeek[1]}W${mWeek[2].padStart(2, '0')}`;
        // 季度：2026Q1
        const mQuarter = key.match(/^(\d{4})Q(\d)$/);
        if (mQuarter) return `${mQuarter[1]}Q${mQuarter[2]}`;
        // 年度：2026
        const mYear = key.match(/^(\d{4})$/);
        if (mYear) return mYear[1];
        // 中文月份：1月
        const mCn = key.match(/^(\d+)月$/);
        if (mCn) return `0000${parseInt(mCn[1]).toString().padStart(2, '0')}`;
        // 过渡格式：2024-01
        const mTrans = key.match(/^(\d{4})-(\d{2})$/);
        if (mTrans) return `${mTrans[1]}${mTrans[2]}`;
        return key;
      };
  ...
  const exportMonthlyExcel = () => {
    ...
    filteredNodes.forEach((node, nodeIndex) => {
      forecastMonthKeys.forEach((monthKey, monthIndex) => {
        ...
        const baselineScenario = baselineScenarioId ? scenarios[baselineScenarioId] : null;
        const baselineNode = baselineScenario?.nodes?.[node.id];
        const baselineValue = findTimeDataValue(baselineNode?.timeData, monthKey);

        selectedScenarios.forEach(scenario => {
          const scenarioNode = scenario.nodes?.[node.id];
          const value = findTimeDataValue(scenarioNode?.timeData, monthKey);
          const isBaseline = scenario.id === baselineScenarioId;
  ...
  const monthlyChartData = useMemo(() => {
    ...
    return forecastMonthKeys.map(month => {
      const item = { month: formatPeriodKey(month) };
      selectedScenarios.forEach(scenario => {
        const scenarioNode = scenario.nodes?.[node.id];
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 flex flex-col overflow-hidden">
            ...
            <div className="h-1/2 flex flex-col">
              <div className="px-3 py-1.5 bg-gray-50 border-b flex items-center justify-between flex-shrink-0">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-semibold text-gray-700">📅 分{getTimeDimensionName(timeDimension).replace("度", "")}对比 (仅预测期)</span>
                  <div className="flex bg-white rounded border overflow-hidden">
                    <button
                    ...
                    <thead className="bg-gray-100 sticky top-0 z-20">
                      <tr>
                        ...
                        <th className="px-2 py-1.5 text-center font-semibold text-gray-600 uppercase tracking-wider border-r border-gray-200 w-20">
                          {getTimeDimensionName(timeDimension)}
                        </th>
                        {selectedScenarios.map(scenario => {
                          const isBaseline = scenario.id === baselineScenarioId;
                          return (
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                      {filteredNodes.flatMap((node) => {
                        return forecastMonthKeys.map((monthKey, monthIndex) => {
                          ...
                          return (
                            <tr
                              key={`${node.id}-${formatPeriodKey(monthKey)}`}
                              className={`hover:bg-gray-50 cursor-pointer ${selectedChartNode === node.id ? 'bg-indigo-50' : ''}`}
                              onClick={() => setSelectedChartNode(node.id)}
                            >
