...
const StdDevAnalysisPanel = ({
  ...
  // 标准差配置（使用本地状态，不依赖 store）
  const [stdDevOptions, setStdDevOptionsLocal] = useState(DEFAULT_STD_DEV_OPTIONS);

  // 封装 setStdDevOptions，支持部分更新
  const setStdDevOptions = useCallback((updates) => {
    setStdDevOptionsLocal(prev => ({ ...prev, ...updates }));
  }, []);
  // 标准差数据（使用本地状态，不依赖 store）
  const [stdDevData, setStdDevData] = useState([]);
  const [selectedStdDevNode, setSelectedStdDevNode] = useState(null);
  ...
  const recalculateStdDev = useCallback(() => {
      ...
      Object.values(nodes).forEach(node => {
        if (node && node.timeData) {
          Object.keys(node.timeData).forEach(key => {
            // 跳过 _cumulative 后缀的 key（它们不是独立的期间）
            if (key.includes('_cumulative')) return;
            // 新格式：2026WK01-AC/FC/BU，提取基础期间键（去掉 -AC/-FC/-BU 后缀）
            const baseKey = key.replace(/-(AC|FC|BU)$/, '');
            // 旧格式：1 月实际/预测/目标
            if (!key.includes('目标')) {
              periodSet.add(baseKey);
            }
          });
        }
      });
    ...
    console.log('[StdDevAnalysisPanel] recalculateStdDev called', {
      nodeCount: nodesArray.length,
      monthsCount: months.length,
      months: months,
      dataMode: stdDevOptions.dataMode,
      compareInitial: stdDevOptions.compareInitial,
      sampleNode: nodesArray.length > 0 ? {
        id: nodesArray[0].id,
        name: nodesArray[0].name,
        type: nodesArray[0].type,
        timeDataKeys: nodesArray[0].timeData ? Object.keys(nodesArray[0].timeData).slice(0, 5) : null,
        hasOriginalTimeData: !!nodesArray[0].originalTimeData,
      } : null
    });
    ...
    console.log('[StdDevAnalysisPanel] results:', {
      ...
      sample: results[0] ? {
        ...
        cvA: results[0].cvA,
        cvB: results[0].cvB,
        isInsufficient: results[0].isInsufficient,
        message: results[0].message,
        totalMonths: results[0].totalMonths,
        quadrant: results[0].quadrant?.name
      } : null,
      insufficientCount: results.filter(r => r.isInsufficient).length,
      validCount: results.filter(r => !r.isInsufficient).length,
      allInsufficient: results.filter(r => r.isInsufficient).slice(0, 3).map(r => ({
        nodeName: r.nodeName,
        message: r.message,
  ...
  // 为数据添加 visual 属性（颜色、形状）
  const stdDevDataWithVisual = useMemo(() => {
    if (!stdDevData || stdDevData.length === 0) return [];
    
    // 颜色配置：根据指标名称分配颜色
    const getColorForIndicator = (nodeName) => {
      const name = nodeName || '';
      if (name.includes('净利润') || name.includes('利润') || name.includes('收益')) return '#1e40af';
      if (name.includes('收入') || name.includes('销售') || name.includes('流量')) return '#16a34a';
      if (name.includes('成本') || name.includes('损耗') || name.includes('损失')) return '#dc2626';
      if (name.includes('毛利') || name.includes('贡献')) return '#9333ea';
      if (name.includes('费用') || name.includes('税金')) return '#ea580c';
      if (name.includes('转化') || name.includes('率')) return '#0891b2';
      if (name.includes('数') || name.includes('量')) return '#f59e0b';
      return '#6b7280'; // 默认灰色
    };
    ...
    // 收集所有唯一的节点名称和方案组合
    const nodeScenarioMap = new Map();
    stdDevData.forEach((item, index) => {
      const key = `${item.nodeName}-${item.scenarioId}-${item.isInitialVersion ? 'initial' : 'current'}`;
      if (!nodeScenarioMap.has(item.nodeName)) {
        nodeScenarioMap.set(item.nodeName, new Set());
      }
      nodeScenarioMap.get(item.nodeName).add(item.scenarioId);
    });

    // 为每个数据点添加 visual 属性
    return stdDevData.map((item, index) => {
      const color = getColorForIndicator(item.nodeName);
      const shapeIndex = Array.from(nodeScenarioMap.keys()).indexOf(item.nodeName) % shapes.length;
      const symbol = shapes[shapeIndex];
      ...
      return {
        ...
        visual: {
          color,
          symbol,
          fill: item.isInitialVersion ? 'light' : 'solid'
        },
        // 为散点图准备的数据格式
        cvA: item.cvA || 0,
        cvB: item.cvB || 0,
        ...
        isInitialVersion: item.isInitialVersion,
  ...
  const filteredStdDevData = useMemo(() => {
    ...
    // 如果没有选择指标，显示所有
    if (selectedIndicators.length === 0) return stdDevDataWithVisual;

    return stdDevDataWithVisual.filter(item => {
      if (!item.nodeName) return false;
      // 直接匹配节点名称，不做类别转换
      return selectedIndicators.includes(item.nodeName);
    });
  }, [stdDevDataWithVisual, stdDevOptions.selectedIndicators]);
