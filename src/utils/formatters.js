 ...
 * @param {string} type - 聚合类型：'SUM'|'AVG'|'MIN'|'MAX'|'COUNT'|'COUNT_NONZERO'|'COUNT_EXISTS'|'DISTINCT'
 * @returns {number} 聚合结果
 */
export function aggregateMonthlyValues(values, type = 'SUM') {
  ...
  switch (type.toUpperCase()) {
    ...
    case 'COUNT_NONZERO':
      return validValues.filter(v => v !== 0).length;
    case 'COUNT_EXISTS':
      return validValues.filter(v => v !== null && v !== undefined).length;
    case 'DISTINCT':
...
export function aggregateTimeData(timeData = {}, aggregationType = 'sum') {
  ...
  // 先收集所有 AC 期间且值不为 0 的，避免 FC 重复计数
  const acPeriods = new Set();
  Object.keys(timeData).forEach((key) => {
    if (key.includes('_cumulative')) return;
    if (key.endsWith('-AC') || key.includes('实际')) {
      const val = parseFloat(timeData[key]);
      // 只有当 AC 值不为 0 时才记录该期间（避免 AC=0 时跳过真实 FC 数据）
      if (!isNaN(val) && val !== 0) {
        const period = key.replace(/-AC$/, '').replace(/实际$/, '');
        acPeriods.add(period);
      }
    }
  });
  ...
  Object.keys(timeData).forEach((key) => {
    ...
    // 过滤掉累计值 key（如 2026WK01-AC_cumulative），只统计单期值
    if (key.includes('_cumulative')) return;

    // 新格式：'2026WK01-AC'，旧格式：'1 月实际'
    const isActual = key.endsWith('-AC') || key.includes('实际');
    const isForecast = key.endsWith('-FC') || key.includes('预测');
    const isTarget = key.endsWith('-BU') || key.includes('目标');

    if (isActual) {
      // 只有当 AC 值不为 0 时才计数（WK13-WK18 的 -AC key 值为 0，不应计入）
      if (value !== 0) {
      ...
      // 关键修复：如果同期间已有 AC 数据，跳过 FC 计数（避免重复计数）
      const period = key.replace(/-FC$/, '').replace(/预测$/, '');
      if (acPeriods.has(period)) return;
