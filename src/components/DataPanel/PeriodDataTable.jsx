...
const PeriodDataTable = ({ nodes, onExportExcel }) => {
  // 按 node.id 去重，防止重复渲染
  const uniqueNodes = {};
  ...
  const tableRows = nodeList.map(node => {
    ...
    const hasPeriodFunction = !isDriver && FormulaParser.hasPeriodFunction(node.formula);
    ...
    if (!isDriver) {
      if (hasPeriodFunction) {
        // 有 PERIOD 函数：实际+预测用 node.value，目标用 node.targetValue
        actualPlusForecastTotal = node.value ?? 0;
      }
      // 无论是否 PERIOD，目标都用 node.targetValue
