...
const FormulaEditorPanel = ({ nodes, onUpdateNode, onAddNode, onDeleteNode, onClose, isMinimized, onToggleMinimize, onMigrateFormulas }) => {
  ...
  const [isCreatingNew, setIsCreatingNew] = useState(false); // 是否正在创建新节点
  ...
  useEffect(() => {
    if (selectedNode) {
      ...
      setIsCreatingNew(false); // 选中已有节点，退出新建模式
    } else {
      const newFormData = {
        id: '',
        name: '',
      ...
      // 不清除 isCreatingNew，让表单保持显示
    }
  }, [selectedNode]);

  // 当名称变化时，自动更新 ID（仅当 ID 为自动生成的 ID 时）
  const handleNameChange = (e) => {
    const name = e.target.value;
    // 只有当 ID 为空或以 'node_' 开头（自动生成的 ID）时，才用名称填充 ID
    // 如果用户已经手动输入了 ID，不再自动覆盖
    const isAutoGeneratedId = !formData.id || formData.id.startsWith('node_');
    const newId = isAutoGeneratedId ? name : formData.id;
  ...
  const handleSave = () => {
    ...
    setIsCreatingNew(false); // 保存后退出新建模式
  ...
  const handleCreateNew = () => {
    setSelectedNodeId(null);
    setIsCreatingNew(true); // 标记为新建模式
    const newFormData = {
      id: generateId(), // 自动生成 ID
      name: '',
      type: 'driver',
      level: '1',
      unit: '',
      ...
      >
        ...
        <div className="flex-1 flex overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {selectedNodeId || isCreatingNew || formData.name ? (
                              ...
                              <div className="flex flex-wrap gap-1">
                                {driverNodes.map(n => (
                                  <button
                                    ...
                                    className="text-xs px-2 py-1 bg-white hover:bg-blue-50 border border-gray-200 rounded font-normal text-blue-700 hover:text-blue-800 shadow-sm"
                                    title={n.id}
                                  >
                                    {n.name}
                                  </button>
