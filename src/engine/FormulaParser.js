...
export class FormulaParser {
  ...
  static replaceMonthlyWithPlaceholder(formula) {
    if (!formula) return { formula, placeholder: null, inner: null, type: null, funcName: null };

    // 递归替换所有 MONTHLY 函数，每个函数使用唯一的占位符
    let result = formula;
    const placeholders = [];
    let placeholderIndex = 0;
    let detected = this.detectMonthlyFunction(result);

    while (detected) {
      // 为每个月函数创建唯一占位符
      const placeholder = `__MONTHLY_${detected.type}_RESULT_${placeholderIndex}__`;
      placeholders.push({
        placeholder,
      ...
      const before = result.substring(0, detected.match.index);
      const after = result.substring(detected.closeParenIndex + 1);
      result = before + placeholder + after;

      // 继续检测是否还有 MONTHLY 函数
      detected = this.detectMonthlyFunction(result);
    }

    // 返回第一个占位符作为主占位符（向后兼容）
    const mainPlaceholder = placeholders.length > 0 ? placeholders[0].placeholder : null;
    const mainInner = placeholders.length > 0 ? placeholders[0].inner : null;
    const mainType = placeholders.length > 0 ? placeholders[0].type : null;
    const mainFuncName = placeholders.length > 0 ? placeholders[0].funcName : null;
  ...
  static extractDependencies(formula, allNodeIds = []) {
    ...
    if (allNodeIds && allNodeIds.length > 0) {
      dependencies = allNodeIds.filter(id => {
        ...
        // 关键修复：使用单词边界匹配，避免子串误匹配
        // 例如：查找 "chengjiaolv" 不应匹配 "chengjiaolv_zong"
        // 使用 (?<![a-zA-Z0-9_]) 和 (?![a-zA-Z0-9_]) 确保完整匹配
  ...
  static compile(formula, allNodeIds = []) {
    ...
    return (nodeValues) => {
      try {
        ...
        // 5. 移除 MONTHLY_* 包装（递归替换所有 MONTHLY 函数）
        // 注意：如果公式包含占位符（如 __MONTHLY_SUM_RESULT_0__），说明 MONTHLY 函数已被替换，需要替换占位符为实际值
        let cleanedExpr = expr;
        const hasPlaceholders = /__MONTHLY_.*_RESULT_\d+__/.test(expr);
        if (hasPlaceholders) {
          // 占位符应该在 nodeValues 中有对应的值，需要替换
          const placeholderRegex = /__MONTHLY_.*?_RESULT_\d+__/g;
          const placeholders = cleanedExpr.match(placeholderRegex);
          if (placeholders) {
            placeholders.forEach(placeholder => {
              const value = nodeValues[placeholder];
              if (value !== undefined && value !== null) {
                const regex = new RegExp(this.escapeRegex(placeholder), 'g');
                cleanedExpr = cleanedExpr.replace(regex, `(${value})`);
              } else {
                // 如果没有对应的值，保留占位符（会导致错误，但这是预期的）
                console.warn(`[FormulaParser] 占位符 ${placeholder} 没有对应的值`);
              }
            });
          }
          expr = cleanedExpr;
        } else {
          let monthlyFunc = this.detectMonthlyFunction(cleanedExpr);
          while (monthlyFunc) {
            const before = cleanedExpr.substring(0, monthlyFunc.match.index);
            const after = cleanedExpr.substring(monthlyFunc.closeParenIndex + 1);
            cleanedExpr = before + '(' + monthlyFunc.inner + ')' + after;
            monthlyFunc = this.detectMonthlyFunction(cleanedExpr);
          }
          expr = cleanedExpr;
        }
