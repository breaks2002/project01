...
function App() {
  ...
  // 暴露获取最新 nodes 的全局函数，供 DataImportModal 使用
  React.useEffect(() => {
    window.__VDT_GET_NODES = () => {
      try { return useVDTStore.getState().nodes; } catch { return {}; }
    };
  }, []);
  ...
  const layoutDirection = useVDTStore(s => s.layoutDirection);
  const toggleLayoutDirection = useVDTStore(s => s.toggleLayoutDirection);
  ...
  const handleSaveNode = useCallback((newNode) => {
    ...
    if (editingNode && editingNode.id && editingNode.id !== newNode.id) {
      ...
      setTimeout(() => {
        ...
        if (state.architectureAdapter) {
          ...
          Object.values(allNodes).forEach(node => {
            const sourceDataNode = state.architectureAdapter.sourceDataManager._convertToSourceDataNode(node, state.architectureAdapter.sourceDataManager.getTimeDimension());
            sourceData.set(sourceDataNode.id, sourceDataNode);
            if (sourceDataNode.periods) {
              Object.keys(sourceDataNode.periods).forEach(p => allPeriodKeys.add(p));
            }
          });

          state.architectureAdapter.sourceDataManager._sourceData = sourceData;
          state.architectureAdapter.sourceDataManager._version++;
          state.architectureAdapter.adjustmentManager.clearAdjustments();
          state.architectureAdapter.formulaEngine.calculateAll(sourceData);
          ...
          Object.keys(calculatedNodes).forEach(nodeId => {
            const originalNode = allNodes[nodeId];
          ...
          console.log('[handleSaveNode] ID 变更后重新计算完成');
        }
      }, 100);
    } else {
      // ID 未变化，正常更新
      ...
      if (nodes[newNode.id]) {
        ...
        // 如果是计算指标，触发重新计算
        if (newNode.type === 'computed' && newNode.formula) {
          setTimeout(() => {
            const state = useVDTStore.getState();
            if (state.architectureAdapter) {
              const allNodes = state.nodes;
              ...
              set({ nodes: calculatedNodes });
              console.log('[handleSaveNode] 重新计算完成');
            }
          }, 100);
        }
      } else {
  ...
  // 处理 CSV/Excel 导入（支持追加/覆盖模式）
  // 将 nodes 对象转换为 CSV 文本
  const nodesToCSVText = useCallback((allNodes) => {
    // 收集所有 period keys
    const allPeriods = new Set();
    Object.values(allNodes).forEach(node => {
      if (node.timeData) {
        Object.keys(node.timeData).forEach(key => {
          const match = key.match(/^(.+)-(AC|FC|BU)$/);
          if (match) allPeriods.add(match[1]);
        });
      }
      if (node.originalTimeData) {
        Object.keys(node.originalTimeData).forEach(key => {
          const match = key.match(/^(.+)-(AC|FC|BU)$/);
          if (match) allPeriods.add(match[1]);
        });
      }
    ...
    // 构建 CSV 文本（使用 BOM 头）
    let csvText = '﻿指标ID,指标名称,节点类型';
    sortedPeriods.forEach(p => { csvText += `,${p}-AC,${p}-FC,${p}-BU`; });
    csvText += '\n';

    Object.values(allNodes).forEach(node => {
      csvText += `"${node.id}","${node.name}","${node.type || 'driver'}"`;
      sortedPeriods.forEach(p => {
        const ac = node.timeData?.[`${p}-AC`] ?? node.originalTimeData?.[`${p}-AC`] ?? '';
        const fc = node.timeData?.[`${p}-FC`] ?? node.originalTimeData?.[`${p}-FC`] ?? '';
        const bu = node.timeData?.[`${p}-BU`] ?? node.originalTimeData?.[`${p}-BU`] ?? '';
        csvText += `,${ac},${fc},${bu}`;
      });
      csvText += '\n';
    });

    return { csvText, formulaText: null };
  }, []);

  const handleCSVImport = useCallback(async ({ dataFile, formulaFile, nodes, mode = 'replace' }) => {
    console.log('[handleCSVImport] 收到参数:', { hasDataFile: !!dataFile, hasFormulaFile: !!formulaFile, hasNodes: !!nodes, mode });

    const state = useVDTStore.getState();

    // 如果有 nodes 对象（Excel 导入），使用 importModel
    if (nodes && state.importModel) {
      try {
        console.log('[handleCSVImport] 使用 importModel 导入, nodes count:', Object.keys(nodes).length);
        const appendMode = mode === 'append';
        state.importModel({ nodes }, { append: appendMode });
        return;
      } catch (error) {
        console.error('importModel 导入失败:', error);
      }
    }
