...
const StdDevScatterChart = ({
  ...
  console.log('[StdDevScatterChart] Received data:', {
    length: data?.length || 0,
    isArray: Array.isArray(data),
    sample: data?.[0] ? { nodeName: data[0].nodeName, isInsufficient: data[0].isInsufficient } : null
  });

  // 过滤掉数据不足的节点
  const validData = useMemo(() => {
    const filtered = data.filter(d => !d.isInsufficient);
    console.log('[StdDevScatterChart] validData debug:', {
      totalData: data?.length || 0,
      validCount: filtered.length,
      insufficientCount: data?.filter(d => d.isInsufficient)?.length || 0,
      sampleInsufficient: data?.filter(d => d.isInsufficient)?.[0]?.message || null,
      sampleValid: filtered[0] ? { nodeName: filtered[0].nodeName, cvA: filtered[0].cvA, cvB: filtered[0].cvB } : null
    });
    return filtered;
  }, [data]);
