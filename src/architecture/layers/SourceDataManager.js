...
import { parseCSV, formulaTableToNodes, parseWideTableToNodes, detectTimeFormat, parseNumericValue } from '../../utils/csvParser';
...
export class SourceDataManager {
  ...
  loadFromCSV(csvText, formulaText = null, existingNodes = new Map()) {
    ...
    if (isWideTable) {
      ...
      csvData.forEach((row, index) => {
        const id = row['指标 ID'] || row['指标名称'];
        if (!id) return;

        const name = row['指标名称'] || id;
        const type = row['节点类型'] || 'driver';
        ...
        // 解析公式表中的 range 配置
        const formulaMin = parseNumericValue(row['最小值']);
        const formulaMax = parseNumericValue(row['最大值']);
    ...
    if (formulaData && formulaData.length > 0) {
      ...
      formulaData.forEach((row, index) => {
        // 兼容 BOM 和空格的列名
        const id = row['指标 ID'] || row['指标 ID'.replace(' ', '')] || row['指标名称'] || '';
        if (!id) {
        ...
        console.log('[SourceDataManager] 公式表 range:', {
          id,
          'formulaMinValue': row['最小值'],
          'formulaMaxValue': row['最大值'],
          'formulaMin': formulaMin,
          'formulaMax': formulaMax,
        ...
        formulaNodes[id] = {
          ...
          id,
          name: row['指标名称'] || (existingNode?.name || id),
          type: row['节点类型'] || (existingNode?.type || 'computed'),
          formula: row['公式'] || '',
          unit: row['单位'] || existingNode?.unit || '',
          format: row['显示格式'] || existingNode?.format || '#,##0',
          direction: row['指标方向'] || existingNode?.direction || 'auto',
          level: row['层级'] || existingNode?.level || '1',
  ...
  _convertToSourceDataNode(node, timeDimension) {
    ...
    allKeys.forEach(key => {
      ...
      if (!match) {
        console.warn(`[SourceDataManager] ${node.id} 无法解析 key: ${key}`);
        return;
      }
