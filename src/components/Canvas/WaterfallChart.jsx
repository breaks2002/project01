...
// 解析月份key，支持多种时间格式：
// 1. "1月实际", "2月预测", "3月目标" (旧格式)
// 2. "2024-01-实际", "2024-02-预测" (过渡格式)
// 3. "202601-AC", "202601-FC", "202601-BU" (新月度格式)
// 4. "20260101-AC" (日度格式)
// 5. "2026WK01-AC" (周度格式)
// 6. "2026Q1-AC" (季度格式)
// 7. "2026-AC" (年度格式)
// 8. "1月-AC", "12月-FC" (中文月份+AC/FC/BU)
  ...
  try {
    ...
    if (matchNew) {
      return {
        month: matchNew[1],
        sortKey: matchNew[1],
        type: matchNew[2] === 'AC' ? '实际' : matchNew[2] === 'FC' ? '预测' : '目标',
        attr: matchNew[2],
        fullKey: key
      };
    }
    // 日度格式：20260101-AC/FC/BU
...
const WaterfallChart = ({ node, allNodes = {}, scenarioName = '当前方案', onClose, canExport, getDisableReason, getAdjustments }) => {
  ...
  // 获取该上下文中所有可用月份（从所有驱动因子的 timeData 中收集）
  const getAllMonths = useCallback((nodesContext) => {
    const monthSet = new Set();
    Object.values(nodesContext).forEach(n => {
      if (n.timeData) {
        Object.keys(n.timeData).forEach(key => {
          const parsed = parseMonthKey(key);
          if (parsed) monthSet.add(parsed.month);
        });
      }
    });
  ...
  // 计算某个节点在指定月份的值（分月计算）
  const computeNodeValueForMonth = useCallback((targetNode, nodesContext, monthKey) => {
        ...
        innerDeps.forEach(depId => {
          ...
          if (depNode) {
            if (depNode.type === 'driver') {
              ...
              monthValues[depId] = computeNodeValueForMonth(depNode, nodesContext, monthKey);
            }
          }
        });
    ...
    deps.forEach(depId => {
      ...
      if (depNode) {
        ...
        if (data && data[monthKey] !== undefined) {
          ...
          valueContext[depId] = computeNodeValueForMonth(depNode, nodesContext, monthKey);
        }
      }
    });
  ...
  // 计算某个节点的聚合值（汇总所有月份）
  const computeAggregatedNodeValue = useCallback((targetNode, nodesContext) => {
    if (!targetNode) return 0;

    if (targetNode.type === 'driver') {
      // 驱动因子：累加 timeData 中所有值
      const data = nodesContext[targetNode.id]?.timeData || targetNode.timeData;
      if (!data) return targetNode.value ?? 0;
      let sum = 0;
      let hasData = false;
      ...
      return hasData ? sum : (targetNode.value ?? 0);
    }

    // 计算指标：先收集所有月份，然后逐月计算后累加
    if (!targetNode.formula) return 0;

    const months = getAllMonths(nodesContext);
    if (months.length === 0) return targetNode.value ?? 0;

    let sum = 0;
    months.forEach(month => {
      // 构造该月的 FC key 来查找数据
      const fcKey = `${month}-FC`;
      const acKey = `${month}-AC`;
      const buKey = `${month}-BU`;

      // 检查该月是否有数据（任意类型）
      let hasMonthData = false;
      Object.values(nodesContext).forEach(n => {
        if (n.timeData && (n.timeData[acKey] !== undefined || n.timeData[fcKey] !== undefined || n.timeData[buKey] !== undefined)) {
          hasMonthData = true;
        }
      });

      if (hasMonthData) {
        sum += computeNodeValueForMonth(targetNode, nodesContext, month);
      }
    });

    return sum;
  }, [allNodes, getAllMonths, computeNodeValueForMonth]);
  ...
  const computeNodeValue = useCallback((targetNode, nodesContext, monthKey = null) => {
    ...
    if (monthKey) {
      // 分月模式：计算单月值
      return computeNodeValueForMonth(targetNode, nodesContext, monthKey);
    } else {
      // 汇总模式：计算聚合值
      return computeAggregatedNodeValue(targetNode, nodesContext);
    }
  }, [computeNodeValueForMonth, computeAggregatedNodeValue]);
  ...
  const computeTargetValue = useCallback((targetNode, monthKey = null) => {
    ...
    if (monthKey) {
      ...
      let targetKey;
      // 支持多种时间格式
      if (/^\d{6,8}$/.test(monthKey)) {
        targetKey = `${monthKey}-BU`;
      } else if (/^\d{4}WK\d{2}$/.test(monthKey)) {
        targetKey = `${monthKey}-BU`;
      } else if (/^\d{4}Q[1-4]$/.test(monthKey)) {
        targetKey = `${monthKey}-BU`;
      } else if (/^\d{4}$/.test(monthKey)) {
        targetKey = `${monthKey}-BU`;
      } else if (monthKey.includes('月')) {
        // 中文月份：优先用新格式，降级用旧格式
  ...
  const waterfallData = useMemo(() => {
    ...
    // 直接使用 store 中架构适配器算好的值，与节点卡保持一致
    // 初始值：使用 initialBaseline（调整前的基准值）
    // 当前值：使用 value（调整后的当前值）
    const initialValue = node.initialBaseline ?? node.value ?? 0;
    ...
    const forecastMonths = months.filter(m => {
      ...
      if (/^\d{6,8}$/.test(m)) {
        keysToTry.push(`${m}-FC`);
      } else if (/^\d{4}WK\d{2}$/.test(m)) {
        keysToTry.push(`${m}-FC`);
      } else if (/^\d{4}Q[1-4]$/.test(m)) {
        keysToTry.push(`${m}-FC`);
      } else if (/^\d{4}$/.test(m)) {
        keysToTry.push(`${m}-FC`);
      } else if (m.includes('月')) {
        keysToTry.push(`${m}-FC`);
        keysToTry.push(`${m}预测`);
      } else {
        keysToTry.push(`${m}-预测`);
      ...
      return keysToTry.some(k => node.timeData && node.timeData[k] !== undefined);
    });
    ...
    // 计算每个驱动因子的影响（仅当有实际调整时）
    const driverImpacts = [];

    // 如果总变化为 0，说明没有做任何调整，直接跳过所有驱动因子影响计算
    if (Math.abs(totalChange) < 0.0000001) {
      return {
        initialValue,
        currentValue,
        targetValue,
    ...
    // 有调整时，只计算真正被调整过的驱动因子的影响
    directDrivers.forEach(driver => {
      // 使用 getAdjustments prop 检查该驱动因子是否有调整
      const hasDriverAdjustment = () => {
        if (!getAdjustments) return true;
        const adjustments = getAdjustments();
        console.log(`[WaterfallChart] ${driver.id}: 调整列表 nodeIds:`, adjustments.map(a => a.nodeId));
        const has = adjustments.some(adj => adj.nodeId === driver.id);
        console.log(`[WaterfallChart] ${driver.id}: hasAdjustment =`, has);
        return has;
      };

      // 未调整则跳过
      if (!hasDriverAdjustment()) {
        console.log(`[WaterfallChart] ${driver.id}: 跳过（未调整）`);
        return;
      }
      console.log(`[WaterfallChart] ${driver.id}: 计算影响`);
  ...
  }, [node, buildInitialContext, buildCurrentContext, getDirectDependencies, computeNodeValue, computeDriverImpact, computeTargetValue, selectedMonth, getAdjustments]);
  ...
  const monthlyWaterfallData = useMemo(() => {
    ...
    // 构造月份key - 支持多种时间格式
    let forecastKey;
    if (/^\d{6,8}$/.test(selectedMonth)) {
      forecastKey = `${selectedMonth}-FC`;
    } else if (/^\d{4}WK\d{2}$/.test(selectedMonth)) {
      forecastKey = `${selectedMonth}-FC`;
    } else if (/^\d{4}Q[1-4]$/.test(selectedMonth)) {
      forecastKey = `${selectedMonth}-FC`;
    } else if (/^\d{4}$/.test(selectedMonth)) {
      forecastKey = `${selectedMonth}-FC`;
    } else if (selectedMonth.includes('月')) {
      // 中文月份：优先用新格式，降级用旧格式
      forecastKey = `${selectedMonth}-FC`;
    ...
    directDrivers.forEach(driver => {
      ...
      const hasDriverAdjustment = () => {
        if (!getAdjustments) return true;
        const adjustments = getAdjustments();
        return adjustments.some(adj => adj.nodeId === driver.id);
      };

      // 未调整则跳过
      if (!hasDriverAdjustment()) return;
  ...
  }, [node, selectedMonth, waterfallData, buildInitialContext, buildCurrentContext, getDirectDependencies, computeNodeValue, computeDriverImpact, computeTargetValue, getAdjustments]);
