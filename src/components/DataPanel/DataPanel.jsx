...
import AdjustmentDescriptionList from './AdjustmentDescriptionList';
...
const DataPanel = ({ nodes, onClose, onOpenFullscreen, currentScenarioName = '', isMinimized, onToggleMinimize, onBringToFront, canExport, isTrial, getDisableReason }) => {
  const [activeTab, setActiveTab] = useState('summary'); // 'summary' | 'detail' | 'descriptions'
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="mb-4 flex-shrink-0">
                ...
                <ul className="text-xs text-gray-500 space-y-1">
                  <li>• <span className="text-green-600">实际汇总</span>：{periodRange.actual || '实际期'}{timeDimensionName}数合计</li>
                  <li>• <span className="text-blue-600">预测汇总</span>：{periodRange.forecast || '预测期'}{timeDimensionName}数合计</li>
                  <li>• <span className="text-green-700 font-semibold">实际 + 预测</span>：实际 + 预测合计</li>
                  <li>• <span className="text-gray-600">目标汇总</span>：{periodRange.target || '全期间'}{timeDimensionName}数合计</li>
                  <li>• <span>差额 (vs 目标)</span>：(实际 + 预测) - 目标</li>
                  <li>• <span>差额 (vs 初始)</span>：(实际 + 预测) - 初始值</li>
                </ul>
              </div>
              <div className="flex-1 overflow-hidden">
                <MonthDataTable nodes={nodes} onExportExcel={handleExportExcel} timeDimension={timeDimension} canExport={canExport} getDisableReason={getDisableReason} />
              </div>
            </div>
          ) : activeTab === 'detail' ? (
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="flex-1 overflow-hidden">
                <MonthDetailTable nodes={nodes} onExportExcel={handleExportExcel} timeDimension={timeDimension} canExport={canExport} getDisableReason={getDisableReason} />
              </div>
            </div>
          ) : (
            <AdjustmentDescriptionList nodes={nodes} onExportExcel={handleExportExcel} canExport={canExport} getDisableReason={getDisableReason} />
          )}
        </div>
      </div>
    </div>
