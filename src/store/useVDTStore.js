...
const useVDTStore = create((set, get) => {
  ...
  return {
    ...
    // 布局方向：'horizontal' 横版（从左到右）| 'vertical' 竖版（从上到下）
    layoutDirection: 'horizontal',

    // 节点列表面板是否可见
    isNodeTreeListVisible: true,
    ...
    importModel: (modelData, options = {}) => {
      ...
      // 重建 SourceData 和计算的通用逻辑（追加和覆盖模式共用）
      const rebuildArchitecture = () => {
        setTimeout(() => {
          const state = get();
          const allNodes = state.nodes || {};
          if (state.architectureAdapter) {
            // 获取已有的 sourceData，用于保留已有的周期数据
            const existingSourceData = state.architectureAdapter.sourceDataManager._sourceData;

            const sourceNodes = new Map();
            const allPeriodKeys = new Set();

            Object.values(allNodes).forEach(node => {
              // 使用 SourceDataManager 的转换方法，与 CSV 导入路径一致
              ...
              sourceNodes.set(sourceDataNode.id, sourceDataNode);
            ...
            // 将 sourceNodes 写入 SourceDataManager
            ...
            console.log('[importModel] rebuildArchitecture 完成，共', Object.keys(calculatedNodes).length, '个节点');
          }
        }, 100);
      };
    ...
    toggleLayoutDirection: () => {
      ...
      const levelSpacing = 620;
      const computedSize = 220;
      const driverSize = 320;
      const startCoord = 100;
      ...
      if (newDirection === 'horizontal') {
        levels.forEach((level, levelIndex) => {
          ...
          const x = startCoord + levelIndex * levelSpacing;
          let y = startCoord;
          sortedNodes.forEach((node) => {
            ...
            y += node.type === 'driver' ? driverSize : computedSize;
          });
        });
      } else {
        const nodeSpacing = 620;
        const rowHeight = 400;
        levels.forEach((level, levelIndex) => {
          const levelNodes = nodesByLevel[level];
          const sortedNodes = [...levelNodes].sort((a, b) => {
          ...
          let x = startCoord;
          const y = startCoord + levelIndex * rowHeight;
          sortedNodes.forEach((node) => {
            newNodes[node.id] = {
              ...newNodes[node.id],
              position: { x, y }
    ...
    rearrangeLayout: () => {
      const state = get();
      const nodesArray = Object.values(state.nodes);
      const layoutDirection = state.layoutDirection;
      ...
      // 网格参数
      const levelSpacing = 620;           // 层级间距
      const computedSize = 220;           // 计算指标尺寸
      const driverSize = 320;             // 驱动因子尺寸
      const startCoord = 100;             // 起始坐标
      ...
      if (layoutDirection === 'horizontal') {
        ...
        // 竖版布局：Y 轴分行，X 轴排列（左对齐，使箭头自然）
        const nodeSpacing = 620;           // 节点间距（X轴，节点520 + 间距100）
        const rowHeight = 400;             // 行高（Y轴间距）
        levels.forEach((level, levelIndex) => {
          const levelNodes = nodesByLevel[level];
          const sortedNodes = [...levelNodes].sort((a, b) => {
            const levelCmp = compareLevels(a.level, b.level);
